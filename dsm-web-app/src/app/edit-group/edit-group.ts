...
import { GroupsService, Group, SocialMedia } from '../services/groups';
...
export class EditGroup implements OnInit {
  ...
  private loadGroup() {
    this.groupsService.getGroups().subscribe(groups => {
      this.currentGroup = groups.find(g => g.id === this.groupId) || null;
      if (this.currentGroup) {
        // Set logo preview if logoUrl exists
        if (this.currentGroup.logoUrl) {
          this.logoPreview = this.currentGroup.logoUrl;
        }

        // Populate form with existing group data
        this.groupForm.patchValue({
          name: this.currentGroup.name,
          location: this.currentGroup.location,
          description: this.currentGroup.description,
          logoUrl: this.currentGroup.logoUrl || '',
          website: this.currentGroup.socialMedia?.website || '',
          meetup: this.currentGroup.socialMedia?.meetup || '',
          facebook: this.currentGroup.socialMedia?.facebook || '',
          twitter: this.currentGroup.socialMedia?.twitter || '',
          linkedin: this.currentGroup.socialMedia?.linkedin || '',
          github: this.currentGroup.socialMedia?.github || '',
          slack: this.currentGroup.socialMedia?.slack || '',
          discord: this.currentGroup.socialMedia?.discord || '',
          youtube: this.currentGroup.socialMedia?.youtube || ''
        });
      }
    });
  }
  ...
  onSubmit() {
    if (this.groupForm.valid && this.currentGroup) {
